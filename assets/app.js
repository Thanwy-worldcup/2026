...
// ============================================================
// ألوان المنتخبات
// الأولوية للون الموجود في Google Sheet
// ============================================================

function teamColor(team) {
  if (!team) return 'var(--gray-team)';

  // لو اللون مكتوب في الشيت بصيغة Hex
  if (
    team.color &&
    /^#[0-9A-Fa-f]{6}$/.test(String(team.color).trim())
  ) {
  ...
  // احتياطي لو اللون مش موجود في الشيت
...
function teamColorHex(team) {
  ...
  // الأولوية للون الموجود في الشيت
  if (
    team.color &&
    /^#[0-9A-Fa-f]{6}$/.test(String(team.color).trim())
  ) {
...
function renderStandingsSummary(teams) {
  ...
  el.innerHTML = sorted.map((t, i) => {
    ...
    // مهم: نرسل المنتخب كامل وليس الاسم فقط
    const hex = teamColorHex(t);

    const chipStyle =
      `--chip-color:${hex};
       --chip-bg:${hexToRgba(hex, 0.06)};
       --chip-border:${hexToRgba(hex, 0.3)};`;
    ...
    return `
      ...
      >
        ...
        ${
          i === 0
            ? `<img class="chip-crown" src="assets/img/crown.png" alt="">`
            : ''
        }
...
function renderTeamsPage(teams) {
  ...
  if (!teams || !teams.length) {
    el.innerHTML = `
      <p class="empty-state">
        لسه مفيش فرق أو لاعبين مسجلين في تاب "المنتخبات"
      </p>
    `;
    return;
  }
...
const ACTIVITY_ICONS = [
  ...
  {
    keys: ['ورشة عمل', 'ورشة إبداعية', 'ورشة', 'ورش'],
    icon: 'workshop'
  },

  {
    keys: ['scope', 'scape', 'سكوب'],
    icon: 'scope'
  },

  {
    keys: ['نشاط حر', 'رسم', 'فني'],
    icon: 'art'
  },

  {
    keys: ['سؤال محيرني', 'سؤال', 'مسابقة'],
    icon: 'idea'
  },

  {
    keys: ['got talant', 'مواهب', 'تالنت'],
    icon: 'idea'
  },

  {
    keys: ['مباراة', 'ماتش', 'كورة', 'تحدي'],
    icon: 'trophy'
  },

  {
    keys: ['جيم', 'رياضة', 'تمرين'],
    icon: 'gym'
  },
  ...
  {
    keys: ['العاب', 'ألعاب', 'game'],
    icon: 'games'
  },

  {
    keys: ['جلسة مسائية', 'سهرة', 'مسائية'],
    icon: 'evening'
  },

  {
    keys: ['مسبح', 'سباحة', 'pool'],
    icon: 'pool'
  },

  {
    keys: ['قهوة', 'ترحيب', 'استقبال'],
    icon: 'coffee'
  },
...
function renderSchedule(schedule) {
  ...
  el.innerHTML =
    Object.entries(groups).map(([date, items]) => {
      ...
      const dayNum =
        dayNumberByDate[date];

      const dayLabel =
        ARABIC_ORDINALS[dayNum - 1] ||
        ('#' + dayNum);
                  ...
                  <div class="timeline-icon">
                    ...
                    ${
                      iconUrl
                        ...
                        : `<span class="timeline-icon-fallback">
                             🗓️
                           </span>`
                    }
...
// ============================================================
// الحدث الحالي أو الأقرب
// ============================================================

function renderNextEvent(schedule) {

  const metaEl =
    document.querySelector('[data-next-meta]');

  const centerEl =
    document.querySelector('[data-next-center]');
  ...
  if (!item) {
    ...
    centerEl.innerHTML =
      '<span>لا يوجد نشاط قادم حاليًا</span>';
  ...
  metaEl.innerHTML = `
    ...
    ${
      live
        ? '<span class="live-tag">جارٍ الآن</span>'
        : ''
    }
...
function renderGallery(photos) {
  ...
  if (!photos || !photos.length) {
    ...
    el.innerHTML = `
      <p class="empty-state">
        لسه مفيش صور مرفوعة في مجلد درايف
      </p>
    `;
...
function renderTopScorers(teams) {
  ...
  const el =
    document.querySelector('[data-top-scorers]');
  ...
  el.innerHTML =
    top3.map((p, i) => {
      ...
      // استخدام لون المنتخب القادم من الشيت
      const color =
        teamColor({
          name: p.team,
          color: p.teamColor
